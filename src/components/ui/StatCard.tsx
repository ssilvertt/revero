...
export function StatCard({ variant }: StatCardProps) {
    ...
    return (
        <div className="relative box-border rounded-[14px] backdrop-blur-custom bg-[rgba(17,17,17,0.55)] py-4 w-full h-[110px] overflow-hidden">
            {variant === 'invest' ? (
                <div className="absolute top-2 left-4 text-white font-proxima">
                    <p className="text-[30px] leading-[110%] tracking-[-2%] font-bold">{data.amount}</p>
                    <p className="text-[15px] font-normal">дней</p>
                </div>
            ) : (
                <p
                    className={`absolute top-2 left-4 text-white font-proxima ${
            ...
            <p className="absolute bottom-2 left-4 z-10 font-proxima text-white text-[15px] leading-[110%] font-light">
                {data.description}
            </p>

            <div className="absolute top-2 right-3 z-0 text-white">
                <div
